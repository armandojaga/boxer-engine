    ...
    */
    ...
    Mesh_A Model_A::ProcessMesh(aiMesh* mesh, const aiScene* scene)
    {
        ...
        for (unsigned int i = 0; i < mesh->mNumVertices; i++)
        {
            ...
            maxPoint.x = std::max(vector.x, minPoint.x);
            maxPoint.y = std::max(vector.y, minPoint.y);
            maxPoint.z = std::max(vector.z, minPoint.z);
        ...
        BoundingBox bb(BB_TYPE::AABB, minPoint, maxPoint);
        return Mesh_A(vertices, indices, textures, bb);
    }
